...
Status: In Progress. Ticket 016A is complete with schema/API persistence foundations for conversations, messages, read receipts, tasks, notifications, and email delivery records.
...
- Ticket 016B - Messages UI Persistence: wire `/messages` to conversations/messages APIs with fixture fallback and persisted send behavior.
- Ticket 016C - Dashboard Tasks And Data: wire Work To-Do and dashboard cards to real APIs for available domains.
