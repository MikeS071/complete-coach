...
Status: In Progress. Tickets 017A through 017E are complete with package/payment schema foundations, tenant-scoped package APIs, Stripe Connect account-link onboarding, trusted Stripe product/price sync, Stripe Checkout client subscription creation, and idempotent Stripe webhook processing; continue with Ticket 017F for API-backed packages UI and revenue persistence.
