...
import {
  buildMessageAttachmentObjectKey,
  getMessageAttachmentMaxBytes,
  messageAttachmentUploadSchema
} from "@/lib/operations/message-attachments";
...
export async function POST(request: Request) {
  try {
    ...
    const input = messageAttachmentUploadSchema.parse(await request.json());
    ...
    const objectKey = buildMessageAttachmentObjectKey(actor.organizationId, input);
    ...
    return dataResponse({
      ...
      maxBytes: getMessageAttachmentMaxBytes()
    });
  } catch (error) {
